...
public abstract class FileLoaderRunner extends DBRunner<LoaderOptions> {
	...
	//Holds a mapping of NAPSID to SiteRecord, shared across threads
	private static final Map<Integer, SiteRecord> siteLookup = new ConcurrentHashMap<Integer, SiteRecord>(300);
	...
	protected SiteRecord getSite(String napsID, String cityName, String provTerr, String latitudeRaw, String longitudeRaw, long recordNumber) {
		try {
			final Integer NAPSID = parseNAPSID(napsID);
			//If one thread stamps overrides the data of another it's no big deal
			return siteLookup.computeIfAbsent(NAPSID, key -> {
				...
				try(SqlSession session = getSqlSessionFactory().openSession(true)) {					
					...
					//Check for the bare minimum data we need, these are all not-null DB columns.
					if(null != cityName && null != provTerr && null != latitudeRaw && null != longitudeRaw) {
					...
					site = mapper.getSiteByNAPSID(NAPSID);
				}
				
				if( null == site) {
					throw new IllegalArgumentException("Could not find matching Site for NAPS ID (" + napsID + ") on row " + recordNumber);
				}
				return site;
			});
	...
	protected SiteRecord getSite(String napsID, long recordNumber) {
		return getSite(napsID, null, null, null, null, recordNumber);
	}
	
	private Integer parseNAPSID(String napsID) {
		if(napsID.contains(".")) {
			//Try as a double
			return (int) Double.parseDouble(napsID);
		} else {
			return Integer.parseInt(napsID);
		}
	}
